    ...
    <section>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6">
        {grades.map((grade) => (
          <Card
            key={grade}
            className="hover:shadow-lg transition-shadow cursor-pointer transform hover:-translate-y-1 duration-300 ease-in-out bg-card rounded-xl overflow-hidden"
            onClick={() => handleGradeSelect(grade)}
          >
            <CardContent className="flex flex-col items-center justify-center p-6 aspect-square">
              <VideoIcon className="h-10 w-10 sm:h-12 sm:w-12 text-primary mb-3" />
              <p className="text-md sm:text-lg font-medium text-center text-card-foreground">
                {grade}
              </p>
            </CardContent>
          </Card>
  ...
  const renderSubjectCard = (subject: string, icon?: React.ReactNode) => (
    ...
    >
      <CardContent className="flex flex-col items-center justify-center p-4 sm:p-6 aspect-square">
        {icon || (
          <VideoIcon className="h-8 w-8 sm:h-10 sm:w-10 text-primary mb-2" />
        )}
        <p className="text-sm sm:text-md font-medium text-center text-card-foreground">
          {subject}
        </p>
      </CardContent>
    </Card>
          ...
          <div className="space-y-8">
            {Object.entries(subjectGroupsForGrade).map(
              ([groupName, subjectsInGroup]) => (
                <div
                  key={groupName}
                  className="p-6 bg-accent rounded-lg shadow"
                >
                  <h3 className="text-xl font-semibold mb-4 text-accent-foreground capitalize">
                    {(() => {
    ...
    <div className="container mx-auto py-12 px-4 sm:px-6 lg:px-8 min-h-screen">
      <div className="max-w-5xl mx-auto">
        <header className="mb-10 text-center">
          <h1 className="text-4xl font-bold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-primary to-green-500 sm:text-5xl">
